...
const setCache = (key: string, value: string) => {
    let cache = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file).toString()) : {};
    cache[key] = value;
...
export default function uniViteStaticProxy({port = 3000, staticPath = '', proxyConfigs = []}): Plugin {
    let app = express();
